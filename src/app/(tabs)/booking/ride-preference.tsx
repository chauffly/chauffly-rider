...
type PillValue = 'cool' | 'neutral' | 'warm' | 'no_music' | 'jazz' | 'classical' | 'afrobeats' | 'quiet' | 'minimal' | 'open' | 'scent_neutral' | 'citrus' | 'wood' | 'floral';
...
export default function RidePreferenceScreen() {
  ...
  const [conversationMode, setConversationMode] = useState<PillValue>('minimal');
  ...
  const renderPills = (options: PillOption[], value: PillValue, onSelect: (val: PillValue) => void) => (
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + spacing.md }]}>
      <View style={styles.header}>
        ...
        <Text variant="h3" font="medium" translationKey="booking.ride_preference" />
      </View>

      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text variant="label" translationKey="booking.preference_cabin_temperature" />
        {renderPills(cabinOptions, cabinTemp, setCabinTemp)}

        <Text variant="label" translationKey="booking.preference_music_genre" />
        {renderPills(musicOptions, musicGenre, setMusicGenre)}

        <Text variant="label" translationKey="booking.preference_conversation_mode" />
        {renderPills(conversationOptions, conversationMode, setConversationMode)}

        <View style={styles.toggleRow}>
          <Text variant="label" translationKey="booking.preference_door_etiquette" />
          <Switch
            value={doorEtiquette}
        ...
        <Text variant="label" translationKey="booking.preference_ambient_scent" />
        {renderPills(scentOptions, ambientScent, setAmbientScent)}

        <Text variant="label" translationKey="booking.preference_in_car_amenities" />
        ...
        <View style={styles.toggleRow}>
          <Text variant="label" translationKey="booking.preference_security_escort" />
          <Switch
